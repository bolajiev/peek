...
export default function AIChatHubScreen() {
  ...
  const openConversation = (conv: Conversation) =>
    navigation.navigate('AIChat', { modelId, conversationId: conv.id });
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          <View style={[styles.heroIcon, { backgroundColor: theme.cardAlt, borderColor: theme.border }]}>
            <IconChat size={26} color={theme.text} strokeWidth={1.6} />
          </View>
          <Text style={[styles.title, { color: theme.text }]}>AI Chat</Text>
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>Ask anything — fully private, on-device.</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  heroIcon: { width: 52, height: 52, borderRadius: 16, borderWidth: 1, justifyContent: 'center', alignItems: 'center', marginBottom: 6 },
